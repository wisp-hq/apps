#!/usr/bin/env bash
# Fail if a PR changes a wisp.json without bumping its version field.
set -euo pipefail
...
changed=$(git diff --name-only "$base_ref" HEAD -- '*/wisp.json' | sort -u)

if [ -z "$changed" ]; then
  echo "no wisp.json changes — skipping version-bump check"
  exit 0
fi
