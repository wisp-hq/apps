...
Translatable strings live in per-app `i18n/<locale>.json` files and are referenced from `app.json` with the `t:` prefix.

### How it works

- Any string in `app.json` may be the literal `"t:<key>"`, which the client resolves from `<slug>/i18n/<defaultLocale>.json`.
...
Every change to a `*/app.json` must bump that manifest's `version` field (semver). The Wisp client uses it to detect catalog updates. CI fails any PR that mutates a manifest without changing its `version`.

Pure rename/move of an i18n file or icon does not require a version bump — only changes to `app.json` content do.
